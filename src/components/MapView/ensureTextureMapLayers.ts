...
import type { TextureFeature } from "@/domain/textures/types";
import {
...
import { ensureTextureMarkerIcons } from "./textureMarkerCanvas";

export const ensureTextureMapLayers = (map: MapLibreMap, texturesForIcons: TextureFeature[] = []) => {
  ...
  ensureTextureMarkerIcons(map, texturesForIcons);
  ...
  if (!map.getLayer(DOT_LAYER_ID)) {
    map.addLayer({
      ...
      paint: {
        "circle-color": ["coalesce", ["get", "__marker_dot_color"], "#ff8500"],
        "circle-radius": ["interpolate", ["linear"], ["zoom"], 4, 2, 12, 3.5, 16, 4],
        "circle-stroke-color": "rgba(18, 18, 18, 0.7)",
