...
export class MerchantMarkerFadeAnimator {
  ...
  sync(
    ...
    for (const feature of targetFeatures) {
      targetById.set(getPartnerId(feature), feature);
    }
    ...
    const step = (now: number) => {
      ...
      for (const [id, track] of this.tracks) {
        if (track.current < track.target) {
          track.current = Math.min(track.target, track.current + stepSize);
        } else if (track.current > track.target) {
          track.current = Math.max(track.target, track.current - stepSize);
        }

        if (Math.abs(track.current - track.target) > 0.01) {
          animating = true;
        } else {
