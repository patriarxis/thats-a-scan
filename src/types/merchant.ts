...
export function getLocalizedField(
  ...
  const suffixPrimary = locale === "el" ? "GR" : "EN";
  const suffixFallback = locale === "el" ? "EN" : "GR";
...
export const getMerchantName = (feature: MerchantFeature, locale: Locale): string =>
  String(
    getLocalizedField(feature.properties, "BrandName", locale) ||
      getLocalizedField(feature.properties, "VATName", locale) ||
      (locale === "el" ? "Κατάστημα" : "Store")
  );

export const getMerchantAddress = (feature: MerchantFeature, locale: Locale): string => {
